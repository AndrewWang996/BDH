...
struct GLMesh
{
    ...
    GLMesh() :vaoHandle(0), showTexture(false), mMeshScale(1.f), mTranslate({ 0.f }), mTextureScale(1.f), edgeWidth(1), pointSize(0), actVertex(-1), actFace(-1), 
        ...
        faceDataDrawScale(0.f), VFDrawScale(0.f), vizVtxData(0)
    {}
    ...
    void draw(const int *vp)
    {
        ...
        if (!constrainVertices.empty()){
            ...
            prog.setUniform("color", 1.f, 0.f, 0.f, .8f);
            prog.setUniform("useTexMap", 0);
            //prog.setUniform("vertexDataScale", 1);

            std::vector<int> idxs = getConstrainVertexIds();
            glDisable(GL_PROGRAM_POINT_SIZE);
    ...
    void main(){
        ...
        gl_Position.z = -VertexTexCoord[1]/100; // todo: remove
    ...
    void main(){
        ...
        if(FragColor.a<1e-2) discard;  // important for transpancy with self overlapping
    })";
